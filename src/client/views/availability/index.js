...
const Availability = () => {
  ...
  const { outboundItinerary, flightSegmentType } =
    !!flightItinerary && flightItinerary[0];
  ...
  const handleSortDirection = () => {
    if (sortDirection === 'asc') setSortDirection('dsc');
  ...
  useEffect(() => {
    if (!!flightSearchInputData) {
      ...
      try {
        dispatch(
          commonAction(endpoint.master.airlines, null, { showLoader: false })
        );
        dispatch(
          commonAction(endpoint.flights.flightSearch, flightSearchInputData, {
  ...
  return (
    <div className="Availability">
          ...
          {!showSearch ? (
            ...
            <div className="Availability-modifySearch__searchSection">
              <div className="d-flex justify-content-between">
                <Text
                  className="modify-text font-primary-bold-20 text-uppercase"
                  text="Modify Search"
                />
                <IconWithBackground
                  showCursor
      ...
      <div
        className={`Availability-mainSection layout-wrapper ${
          loaderStatus.items && !loaderStatus.items.data.isLoaderVisible
            ? 'adjust-padding'
            : ''
        }`}
      >
                ...
                <div className="info-sort-section d-flex justify-content-between align-items-center">
                  {!!totalfareDetails && (
                    <div className="d-flex">
                      <Text
                        className="font-primary-medium-14 mr-4"
                        text="All Amounts in "
                      />
                      <Text
                        className="font-primary-bold-14"
                        text={totalfareDetails.totalAmountCurrency}
                      />
                    </div>
                  )}
                  <div className="sort d-flex align-items-center ml-auto">
                    <SwapVertIcon
                      className="sort-reverse cursor-pointer"
