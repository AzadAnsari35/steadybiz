...
const PopoverAction = ({ rowNumber }) => {
  ...
  const handleClick = (e) => {
    ...
    switch (selectedOption) {
      case 'view': {
        ...
        //   return;
        // }
        utils.setItemToStorage('selectedRegion', rowNumber);
        history.push(routes.master.viewRegion);
        break;
      }
      case 'modify': {
        // const securityMessage = utils.checkSecurityGroup(
        ...
        // }
        utils.setItemToStorage('selectedRegion', rowNumber);
        history.push(routes.master.modifyRegion);
        break;
      }
  ...
  return (
    <>
      ...
      >
        <div
          className="SearchRegion-tableAction d-flex flex-direction-column"
          onClick={handleClick}
        >
...
const SearchRegion = () => {
  ...
  const securityGroupList = useSelector((state) => state.SearchRegion?.items);
  ...
  const callSearch = (page) => {
    try {
      dispatch(
        commonAction(endpoint.office.SearchRegion, {
          ...requestJson,
          page: page - 1,
          size,
          ofId,
  ...
  const onSubmit = (data, e) => {
    const securityMessage = utils.checkSecurityGroup(
      securityOptionConstant.office.SearchRegion
    );
  ...
  const handleCreate = () => {
    ...
    //   return;
    // }
    history.push(routes.master.createRegion);
    utils.setItemToStorage('selectedRegion', null);
  };

  return (
    <div className="SearchRegion">
      <div className="SearchRegion-head">
