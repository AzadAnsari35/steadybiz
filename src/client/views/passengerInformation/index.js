...
const PassengerInformation = () => {
	...
	const [visiblePanelsState, setVisiblePanelsState] = useState(visiblePanelsInitialState);
	const [showFlightDetailContinue, setShowFlightDetailContinue] = useState(true);
	const [showAlert, setShowAlert] = useState(true);
	const [createPNRProcessing, setCreatePNRProcessing] = useState(false);
	const [timeLimitContinueProcessing, setTimeLimitContinueProcessing] = useState(false);
	const [bookingProcessing, setBookingProcessing] = useState(false);
	const [mode, setMode] = useState("");

	const masterCountries = useSelector((state) => state[endpoint.master.countries.reducerName]);
	const masterAirlines = useSelector((state) => state[endpoint.master.airlines.reducerName]);
  const transactionData = useSelector((state) => state[endpoint.transaction.airprice.reducerName]);
  const flightSelectData = useSelector(state => state[endpointWithoutApi.flights.flightSelect.reducerName]);
  const flightSearchInput = useSelector(state => state.flightSearchInput);
  const passengerInformationAllData = useSelector(state => state[endpointWithoutApi.common.passengerInformationData.reducerName]);
  ...
  const passengerInformationData = getDataFromRedux(passengerInformationAllData);
  const transaction = getDataFromRedux(transactionData);
  
  const flightSelect = getDataFromRedux(flightSelectData);
	const { flightItineraryValidate, flightReValidate, outboundItinerary, paxCount } = flightSelect;
  ...
  useEffect(() => {
	...
	const handleMultiplePanelToggle = (currentPanelId, nextPanelId, nextPanelValue) => {
		setPanelsState({
			...panelsState,
			[currentPanelId]: !panelsState[currentPanelId],
			[nextPanelId]: !!nextPanelValue ? nextPanelValue : !panelsState[nextPanelId],
		});
	};
    ...
    try {
      ...
      console.log('err', err);
		...
		handleContinue(currentPanelId, nextPanelId, nextPanelValue, visiblePanelName, visiblePanelValue, formId, formData);
		handleMultiplePanelToggle(currentPanelId, nextPanelId, nextPanelValue);
		setTimeout(() => {
          ...
          {!!alertData && showAlert && (
            <div className="alert-section mb-66" ref={alertRef}>
              <Alert
                ...
                alertMessage={alertData.showMessage && (alertData.Message || alertData.message)}
                showSecondaryAction
              ...
              {visiblePanelsState.isPassengerDetailsPanelVisible && (
                <Panel
                  ...
                  panelHeaderIcon={
                    <Image
                      altText="passenger"
                      imgName="passenger.svg"
                    />
                  }
                  panelIconMarginLeft={"12"}
                  headerText={`PASSENGER : ${paxCount.adultCount > 0 && "ADT"} ${
