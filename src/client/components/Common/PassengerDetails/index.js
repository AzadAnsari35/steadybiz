...
const PassengerDetails = (props) => {
  ...
  if (paxCount.infantCount) {
    for (let i = 0; i < paxCount.infantCount; i++) {
      initialPassengersFormDataState.push({
        id: Object.keys(initialActivePassengerIds)[
          paxCount.adultCount + paxCount.childCount + i
        ],
        typeText: `INFANT ${i + 1}`,
        type: 'INF',
  ...
  const [activePassengersList, setActivePassengersList] = useState(
    initialActivePassengerIds
  );
  const [passengersFormData, updatePassengersFormData] = useState(
    initialPassengersFormDataState
  );
  const [passengersErrorData, updatePassengersErrorData] = useState(
    initialPassengersErrorDataState
  );

  const handleFocus = (id) => {
    const clonedPassengersFormData = [...passengersFormData];
    const updatedPassengerIndex = passengersFormData.findIndex(
    ...
    for (let i = 0; i < passengersFormData.length; i++) {
      formattedData.push({
        passengerInfo: {
          ...
          dateOfBirth: passengersFormData[i].dob
            ? moment(passengersFormData[i].dob).format('YYYY-MM-DD')
            : '',
          passportNumber: passengersFormData[i].passportNumber
            ? passengersFormData[i].passportNumber
            : null,
          issuingCountry: passengersFormData[i].issuingCountry
            ? passengersFormData[i].issuingCountry
            : null,
  ...
  const validateForm = () => {
    ...
    passengersFormData.forEach((passenger, index) => {
      !passenger.title.trim() &&
        (errorObj[index].title = ERROR_MESSAGE.TITLE_REQUIRED);
      !passenger.firstName.trim() &&
        (errorObj[index].firstName = ERROR_MESSAGE.FIRST_NAME_REQUIRED);
      passenger.title.trim() &&
        passenger.firstName.trim() &&
      ...
      !passenger.lastName.trim() &&
        (errorObj[index].lastName = ERROR_MESSAGE.LAST_NAME_REQUIRED);
      passenger.lastName.trim() &&
      ...
      if (!validatePassportFormData(passenger) || paxCount.infantCount > 0) {
        !passenger.passportNumber.trim() &&
          (errorObj[index].passportNumber =
            ERROR_MESSAGE.PASSPORT_NUMBER_REQUIRED);
        !passenger.issuingCountry &&
          (errorObj[index].issuingCountry =
            ERROR_MESSAGE.ISSUING_COUNTRY_REQUIRED);
        !passenger.passportExpiry &&
          (errorObj[index].passportExpiry =
            ERROR_MESSAGE.PASSPORT_EXPIRY_REQUIRED);
        !passenger.nationality &&
          (errorObj[index].nationality = ERROR_MESSAGE.NATIONALITY_REQUIRED);
      } else {
        errorObj[index].passportNumber = '';
        errorObj[index].issuingCountry = '';
        errorObj[index].passportExpiry = '';
      ...
      if (!validateFrequentFlyerFormData(passenger)) {
        !passenger.frequentFlyerAirline &&
          (errorObj[index].frequentFlyerAirline =
            ERROR_MESSAGE.AIRLINE_REQUIRED);
        !passenger.frequentFlyerNumber.trim() &&
          (errorObj[index].frequentFlyerNumber =
            ERROR_MESSAGE.FREQUENT_FLYER_NUMBER_REQUIRED);
      } else {
        errorObj[index].frequentFlyerAirline = '';
        errorObj[index].frequentFlyerNumber = '';
      }
        ...
        {passengersFormData.map((passenger, index) => (
          ...
          >
            ...
            <PassengerPassportInformation
              ...
              onFocus={handleFocus}
              onChange={handleChange}
              infantCount={paxCount.infantCount}
              flightDate={paxCount.flightDate}
            />
          </FormPanel>
        ))}
      </div>
