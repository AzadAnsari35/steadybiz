...
const PrimaryAccordion = props => {
  ...
  return (
    <div className="PrimaryAccordion width-100">
      <div
        className={
          `PrimaryAccordion-header d-flex justify-content-between align-items-center cursor-pointer ${!open ? "mb-0" : ""}`
        }
        onClick={setOpen}
      >
        <Text className="font-primary-semibold" text={text} />
        <ArrowIcon size={12} color={colors.black} orientation={open ? -90 : 90} />
      </div>
      {open &&
        <div className="PrimaryAccordion-content">
          {children}
        </div>
      }
    </div>
